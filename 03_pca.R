...
ref_pyros_pca <- pyrome_pca_df %>% 
  group_by(cell_hist) %>% 
  summarize(across(c(pca1, pca2, cbi, ndvi, frp_cell),
...
frp_pca <- 
  ...
  geom_hline(aes(yintercept = 0)) +
  geom_vline(aes(xintercept = 0)) +
  ggstar::geom_star(aes(fill = frp_cell_mean), color = 'black', starshape = "hexagon", size = 2) +
  geom_segment(data = loadings, 
               aes(x = 0, y = 0, xend = (PC1), yend = (PC2)), 
               arrow = arrow(length = unit(1/2, "picas")),
               size = 1) +
  geom_label_repel(data = loadings, aes(x = PC1, y = PC2),
                   label = c('Burn severity', 'Vegetation productivity','FRP'),
                   fill = c('grey'), alpha = 0.8, min.segment.length = 1, size = 2.5) +
