...
# MAY REMOVE IF CLIMATE BIN VERSION WORKS ------------------------------------
...
# ------------------------------------------------------------------------------
# Burned area  
# Add information about whether all forested points are burned and the area of each cell
# ------------------------------------------------------------------------------
# Record whether forest points burned or not
forest_burned <- terra::extract(burned_forest_rast, forest_pts_sp)

forest_cell_area <- cellSize(burned_forest_rast)
...
sum(forest_burned$mtbs_rast*forest_area_df$area)*0.000001
...
forest_clim_df <- forest_pts_sp %>% 
  ...
  cbind(forest_climate,
        ., 
        'burned' = forest_burned$mtbs_rast, 
