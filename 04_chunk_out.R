...
# ------------------------------------------------------------------------------
pyrome_df <- pyrome_df %>% 
  # Two rows persist where FRI = NA, has to do with resampling/rasterizing
  filter(!is.na(fri))
...
rescale_pyrome_df <- pyrome_df %>% 
  dplyr::select(cell_hist, cell_2C, cbi, ndvi, fri, frs) %>% 
  mutate(across(c(cbi, ndvi, fri, frs), ~ rescale0100(.x), .names = 'z_{.col}'))
